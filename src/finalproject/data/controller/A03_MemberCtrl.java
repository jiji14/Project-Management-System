...
public class A03_MemberCtrl {
	...
	public String login(@ModelAttribute("mem") Member mem, HttpSession session, Model d) {
		...
		if(mem.getId()!=null) { //mem의 여러 정보중 id를 가져오거나 pw를 가져오고있다.
			...
			System.out.println("mem(세션):"+m.getId());
			System.out.println("로그인할때"+m.getMemno());
			System.out.println("로그인할때"+m.getPrjno());
			String auth = service.getAuth(m);
			m.setAuth(auth);
			System.out.println(m.getAuth());
			System.out.println(m.getPrjno());
