...
public class A04_BoardCtrl {
   ...
   public String insert(@ModelAttribute("board") Board ins, HttpSession session, Model d) {
	...
	System.out.println("받은 세션 prjno : "+mem.getPrjno());	
      ...
      d.addAttribute("boardins",service.boardname(memno));
      //DB입력 처리시
   ...
   public String detail(@RequestParam("no") int no, Model d, @ModelAttribute("update") Board update, HttpSession session) {
	   Member mem = (Member)session.getAttribute("mem");
		String memno = mem.getMemno();
		  String name = mem.getName();
		  System.out.println(name);	   
      ...
      d.addAttribute("boardup",service.boardname(memno));  
