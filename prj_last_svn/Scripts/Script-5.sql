...
INSERT INTO uiformat values('uino1', 'memno1000', '#risk', '#project', '#board', '#memLog');
SELECT * FROM membership;
SELECT * FROM members WHERE PRJNO='pro10102';
SELECT * FROM project;

SELECT * FROM risk;

SELECT name, a.prjno prjno, auth FROM members a, membership b
WHERE a.prjno = 'pro10102' AND a.ACTIVATION = '활성화' AND a.memno = b.memno AND b.memno = 'memno1000';

      insert into Membership values('memno'||memno1_seq.nextval,
         'bo@naver.com','12345','this',sysdate,'','','pro0',sysdate);
		SELECT* FROM membership;
		
SELECT name, auth FROM members a, membership b WHERE a.memno = b.memno AND a.memno='memno1000'
AND a.prjno='pro10140';
...
SELECT * FROM membership;
SELECT * FROM UIFORMAT;
SELECT memno1_seq.currval FROM dual;

CREATE SEQUENCE ui_seq
	START WITH 2
	INCREMENT BY 1;

SELECT ui_seq.currval FROM dual;
DROP SEQUENCE ui_seq;
INSERT INTO uiformat(uino, memno) values('uino'||ui_seq.nextval, 'memno1001');


SELECT a.* FROM project a, (SELECT * FROM members WHERE memno='memno1003') b WHERE a.prjno=b.prjno;
UPDATE SET uiformat 
...
where CONSTRAINT_NAME = 'SCOTT.SYS_C007438';

ALTER TABLE project add(activation varchar2(50));
UPDATE project SET activation ='비활성화';
UPDATE project SET activation ='활성화' WHERE prjno='pro10100';
UPDATE project SET activation ='활성화' WHERE prjno='pro10101';
UPDATE project SET activation ='활성화' WHERE prjno='pro10102';
SELECT * FROM project WHERE activation='활성화';
...
dept varchar2(50),
job varchar2(50),
prjno varchar2(50) DEFAULT 'prjno0',
updated DATE
);
...
,to_date('2019-08-02','YYYY-MM-dd'),sysdate,'IUUU','ITBANK','개인프로젝트','memno1000','어쩌란말이냐', '활성화');
INSERT INTO project values('pro'||project_seq.nextval,'제목이라니','진행중','고객님이라니',to_date('2019-08-02','YYYY-MM-dd')
,to_date('2019-08-15','YYYY-MM-dd'),sysdate,'총책임자라니','카테고리필요하니?','팀프로젝트','memno1001','내용이라니', '활성화');
INSERT INTO project values('pro'||project_seq.nextval,'두썸띵','보류','호갱님',to_date('2019-03-18','YYYY-MM-dd')
,to_date('2019-08-09','YYYY-MM-dd'),sysdate,'회사어딘가','ITBANK','팀프로젝트','memno1002','나좀뽑아죠요', '활성화');
INSERT INTO project values('pro'||project_seq.nextval,'유엔빌리지','완료됨','대상감',to_date('2019-07-10','YYYY-MM-dd')
,to_date('2019-08-02','YYYY-MM-dd'),sysdate,'배쿠','ITBANK','개인프로젝트','memno1003','노래짱조아', '활성화');
...
CREATE TABLE members(
   ...
   activation varchar2(50),
   uptdate DATE,
   CHECK (auth IN('admin','member')) ENABLE,  
   CHECK (activation IN('활성화', '비활성화')) enable,
...
CREATE TABLE tasks(
   ...
   isHidden NUMBER DEFAULT 0,						-- todolist에서 쓸 칼럼 (0: false 1: true)
   FOREIGN KEY(prjno) REFERENCES project(prjno),
...
(SELECT (select 
       ROUND(extract(day from (systimestamp - to_date('01011970', 'ddmmyyyy'))) * 24 * 3600 * 1000
...
from dual) - enddate FROM tasks) <= 604800000;
...
INSERT INTO tasks values('task'||task_seq.nextval,'pro10100','수니업무',0,'백앤드 개발 수정입니다.','','STATUS_ACTIVE','',1396994400000,1563763781726,5,0,0,'assigs1','높음',sysdate,'승인', 0);
INSERT INTO tasks values('task'||task_seq.nextval,'pro10100','수니업무2',0,'백앤드 개발 수정입니다2.','','STATUS_ACTIVE','',1396994400000,1463763781726,5,0,0,'assigs1','낮음',sysdate,'승인', 0);
INSERT INTO tasks values('task'||task_seq.nextval,'pro10100','수니업무3',0,'백앤드 개발 수정입니다3.','','STATUS_ACTIVE','',1396994400000,1503763781726,5,0,0,'assigs1','보통',sysdate,'승인', 0);
INSERT INTO tasks values('task'||task_seq.nextval,'pro10100','수니업무4',0,'백앤드 개발 수정입니다4.','','STATUS_ACTIVE','',1396994400000,1543763781726,5,0,0,'assigs1','높음',sysdate,'승인', 0);
SELECT * FROM tasks;

SELECT sysdate-enddate FROM task;
