...
%>
	...
	<form:form commandName="tasks" method="post" >	
		...
		<div class="input-group mb-3">
            ...
            value="승인" id="confirmBtn"/>   
            <input  type="button" class="btn btn-info" 
            value="반려" id="rejectBtn"/>   
	...
	$(document).ready(function(){
		...
		$("#confirmBtn").click(function(){
			...
			if(confirm("승인 하시겠습니까?")){
				$.ajax({
					type:"post",
					url:"${path}/permit.do?method=confirmProc&taskId="+taskid,
					dataType:"json",
			...
			window.opener.location.replace("${path}/permit.do?method=admin");
			window.close();
		});
		...
		$("#rejectBtn").click(function(){
			...
			if(confirm("반려 하시겠습니까?")){
				$.ajax({
					type:"post",
					url:"${path}/permit.do?method=rejectProc&taskId="+taskid,
					dataType:"json",
					success:function(data){
						//페이지띄우는 자바스크립트작성
						if(data.isSucc == "ok"){
