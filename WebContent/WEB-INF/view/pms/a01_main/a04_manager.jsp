        		...
        		<table class="table table-hover" id="permit_tab">
						<thead class="thead-light">
							...
							<c:forEach var="per" items="${pmchk}">
								...
								<td><p>${per.memno}</p></td>
								<td><input  type="button" class="btn btn-primary manageBtn" value="${per.isread}"/></td>
								</tr>
							</c:forEach>
					</table>
        		...
        		<!-- <div class="btn btn-outline-secondary choice" id="choice_2">+ 더보기</div> -->
        			<table class="table table-hover" id="permit_tab">
						<thead class="thead-light">
							...
							<c:forEach var="per" items="${pmreject}">
								<tr class="permitList" id="${per.taskid}">
								<td><p>${per.taskid}</p></td>
								<td><p>${per.prjno}</p></td>
								<td><p>${per.name}</p></td>
        		...
        		<!-- <div class="btn btn-outline-secondary choice" id="choice_3">+ 더보기</div> -->
        			<table class="table table-hover" id="permit_tab">
						<thead class="thead-light">
							...
							<c:forEach var="per" items="${pmdone}">
								<tr class="permitList" id="${per.taskid}">
								<td><p>${per.uptdate}</p></td>
								<td><p>${per.prjno}</p></td>
								<td><p>${per.name}</p></td>
...
//		alert($(this).parents(".permitList").attr("id"));
		...
		$.ajax({
			...
			success:function(data){
				//페이지띄우는 자바스크립트작성
				if(data.isSucc == "ok"){
					var win = window.open("${path}/permit.do?method=manageDatail&taskId="+taskId,"permitDetail","width=900, height=800");
					win.moveTo(500,100);
				}else{
...
//		alert($(this).parents(".permitList").attr("id"));
		...
		window.open("${path}/permit.do?method=detail&taskId="+taskId,"permitDetail","width=900, height=800");
		win.moveTo(500,100);	
	});
